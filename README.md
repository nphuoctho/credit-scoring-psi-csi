...
|---|---|---|
| AUC (held-out) | **0.807** | **0.841** |
| KS | 0.479 | 0.533 |
| Gini | 0.614 | 0.683 |
| 5-fold CV AUC | 0.808 ± 0.010 | 0.842 ± 0.007 |

- **Decision @ 5% FPR:** **SHIP challenger** — catches **+5.6pp** more defaulters (recall 51.1% vs 45.5%; 95% CI **[4.5, 6.8]pp**, paired bootstrap).
- **Economics:** ~**283** extra defaulters caught, ~**$1.4M** loss avoided per window, at ~**$0** extra false-positive cost (equal FPR).
...
Mô phỏng vòng đời thật của một mô hình chấm điểm tín dụng: huấn luyện Logistic Regression (champion, dùng WoE — minh bạch) và LightGBM (challenger — mạnh hơn), quyết định **ship/hold** bằng khung champion-challenger với bootstrap CI tại ngưỡng FPR 5%, rồi **giám sát drift sau triển khai** bằng PSI/CSI. Toàn bộ công thức lõi (WoE/IV, KS, PSI/CSI, bootstrap) đều tự code trong `src/`. Kết quả: challenger bắt thêm **+5.6pp** khách vỡ nợ ở cùng mức từ chối → **ship**; lớp monitoring **bắt đúng drift đã inject** vào `DebtRatio`.
