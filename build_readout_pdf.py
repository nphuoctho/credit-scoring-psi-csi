...
def _fmt_money(x):
    # escape $ so matplotlib's text layer does not read it as a mathtext delimiter
    return f"\\${x:,.0f}"
...
def build():
    ...
    T(0.855,
      ...
      f"champion {dec['champ_recall']:.1%} is +{dec['lift_pp']:.1f}pp more defaulters "
    ...
    T(0.82,
      ...
      f"Rule (pre-registered): ship if lift is at least {dec['min_lift_pp']:.0f}pp and "
      f"the CI lower bound stays above 0, {'met' if ship else 'not met'}.", size=9)

    T(0.78, "2. Risk and cost trade-off", size=11, weight="bold")
    T(0.76,
      ...
      f"False-positive cost about \\$0, because both models are compared at an identical "
      f"5% FPR, so the same share of good customers is declined. Net about "
